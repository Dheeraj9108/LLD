...
public class CommentaryManager {
    ...
    public String generateCommentary(Ball ball) {
        ...
        String batsmanName = ball.getPlayedBy() != null ? ball.getPlayedBy().getName() : "";
    ...
    private String getEventKey(Ball ball) {
        if (ball.isWicket()) {
            return "WICKET_" + ball.getWicket().getType().toString();
        }
        if (ball.getType() != null) {
            return "EXTRA_" + ball.getType().toString();
        }
        if (ball.getRunScored() >= 0 && ball.getRunScored() <= 6) {
            switch(ball.getRunScored()) {
