...
import OnlineShoppingSystemLLD.stratergy.CreditCard;

public class OnlineShoppingSystemLLDMain {
    public static void main(String[] args) {
        // --- System Setup (Singleton and Services) ---
        OnlineShoppingSystemService system = OnlineShoppingSystemService.getInstance();

        // --- Create and Add Products to Catalog (Builder Pattern) ---
        Product laptop = new Product("Dell XPS 15","A powerful and sleek laptop.", 1499.99,ProductCategory.HOME);
        Product book = new Product("The Pragmatic Programmer", "A classic book for software developers.",45.50,ProductCategory.FOOD);
        ...
        System.out.printf("Alice added a book. Original price: $%.2f, New price: $%.2f%n",
                book.getPrice(), book.getPrice());

        Cart aliceCart = system.getCustomerCart(alice.getId());
        System.out.printf("Alice's cart total: $%.2f%n", aliceCart.getTotalPrice());

        // --- Alice Checks Out ---
        System.out.println("\n--- Alice proceeds to checkout ---");
        Order aliceOrder = system.placeOrder(alice.getId(), new CreditCard());
        if (aliceOrder == null) {
        ...
        Order bobOrder = system.placeOrder(bob.getId(), new CreditCard());
        if (bobOrder == null) {
